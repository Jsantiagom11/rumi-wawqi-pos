...
function normalizeOrder(order) {
  ...
  if (!Number.isFinite(cantidad) || cantidad <= 0 || !Number.isFinite(precio) || precio < 0) return null;
  return {
    ...deepClone(order),
    cantidad,
...
export function commitShiftFinalization(database, preparedBackup, now = new Date()) {
  ...
  if (
    ...
    throw new DomainError('STALE_BACKUP', 'Las ventas cambiaron después de generar el backup. Genere uno nuevo.');
  }
...
function checksum(value) {
  return `fnv1a32:${hashText(canonicalize(value)).toString(16).padStart(8, '0')}`;
}
