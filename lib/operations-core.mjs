...
export function computeProductCapacity(input, product) {
  ...
  const capacity = missingIngredients.length === 0 && constraints.length > 0
    ? Math.min(...constraints.map((entry) => entry.plates))
    : null;
  return {
    productId: product?.id ?? recipe.productId,
    productName: product?.nombre ?? recipe.productName ?? 'Producto',
    capacity,
    source: missingIngredients.length === 0 ? 'derived-bom' : 'incomplete-bom',
    constraints,
    missingIngredients,
  };
}
