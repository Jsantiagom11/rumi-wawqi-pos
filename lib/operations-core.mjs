...
function normalizeIngredient(value) {
  ...
  const openingQty = value.openingQty === null || value.openingQty === undefined
    ? null
    : normalizeQty(value.openingQty);
  const reorderPoint = normalizeQty(value.reorderPoint ?? 0);
  const targetQty = normalizeQty(value.targetQty ?? reorderPoint ?? 0);
  if ((value.openingQty !== null && value.openingQty !== undefined && openingQty === null)
      || reorderPoint === null || targetQty === null) return null;
  return {
    ...deepClone(value),
    id: value.id.trim(),
...
export function computeIngredientStock(input, ingredientId) {
  ...
  if (!countAnchor && ingredient.openingQty === null) {
    return {
      ingredientId,
  ...
  const activeAfterAnchor = activeOrders(db).filter((item) => {
    const millis = eventTimestamp(item);
    return millis === null || millis > anchorMillis;
  });
  stock -= quantityForIngredient(activeAfterAnchor, db.operations.recipes, ingredientId);
  ...
  return {
    ...
    initialized: true,
    anchor: countAnchor
      ? { type: 'count', id: countAnchor.entry.id, at: countAnchor.entry.at }
      : { type: 'opening', at: ingredient.openingAt },
  };
}
...
export function computeProductCapacity(input, product) {
  ...
  const legacyCapacity = Number.isFinite(legacy) ? Math.max(0, Math.floor(legacy)) : null;
  if (!recipe) {
    return {
      ...
      constraints: [],
      missingIngredients: [],
      uninitializedIngredients: [],
    };
  }

  const constraints = [];
  const missingIngredients = [];
  const uninitializedIngredients = [];
  for (const component of recipe.components) {
    try {
      const stock = computeIngredientStock(db, component.ingredientId);
      if (!stock.initialized) {
        uninitializedIngredients.push(component.ingredientId);
        continue;
      }
      constraints.push({
      ...
      if (error instanceof OperationsError && error.code === 'UNKNOWN_INGREDIENT') {
        missingIngredients.push(component.ingredientId);
        continue;
      }
      throw error;
    }
  ...
  let capacity = null;
  let source = 'derived-bom';
  if (missingIngredients.length > 0) {
    source = 'incomplete-bom';
  } else if (uninitializedIngredients.length > 0) {
    source = 'legacy-awaiting-count';
    capacity = legacyCapacity;
  } else if (constraints.length > 0) {
    capacity = Math.min(...constraints.map((entry) => entry.plates));
  }
...
export function buildOperationsDashboard(input) {
  ...
  const risks = ingredientStock
    .filter((entry) => entry.initialized && entry.quantity < 0)
    .map((entry) => ({
      code: 'NEGATIVE_STOCK',
  ...
  for (const capacity of capacities) {
    if (capacity.source === 'incomplete-bom') {
      risks.push({
        code: 'INCOMPLETE_BOM',
        productId: capacity.productId,
        message: `${capacity.productName}: faltan ingredientes configurados (${capacity.missingIngredients.join(', ')})`,
      });
    }
    if (capacity.source === 'legacy-awaiting-count') {
      risks.push({
        code: 'COUNT_NEEDED',
        productId: capacity.productId,
        message: `${capacity.productName}: falta un conteo inicial para activar stock derivado`,
      });
    }
  }
